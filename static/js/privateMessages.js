...
function retrevePrivateMessage(from) {
    let privateMessageList = document.getElementById("broadcasts");

    let payload = {
        'request': 'retreive_private_messages',
        'message_from': from,
    };
    ...
    fetch('/updates/retreive_private_messages', options).then(response => {
        return response.json();
    }).then((md_messages) => {
        let list = document.getElementById('broadcasts');
        list.innerHTML = md_messages;
        console.log("Retrieved private messages")
    }).catch(() => {
...
function LoadPrivateMessage(selectionBox) {
    let selection = selectionBox.value;
    retrevePrivateMessage(selection)
}